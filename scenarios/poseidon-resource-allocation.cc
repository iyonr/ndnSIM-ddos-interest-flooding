...
int main (int argc, char**argv)
{
  ...
  if (prefix == "resource-allocation")
    {
      //helper.EnableLimits (true, defaultRtt);  
      helper.SetForwardingStrategy ("ns3::ndn::fw::BestRoute::Stats::PoseidonResourceAllocation");
    }
  else
    {
  ...
  while (evils.size () < badCount)
    {
      ...
      if(name.substr(8,1) == "5" || name.substr(8,1) == "4" || name.substr(8,1) == "6")// || name.substr(8,1) == "9")
      {
