        ...
        for algorithm in self.algorithms:
            for topology in self.topologies:
                for evil in self.evils:
                    for run in self.runs:
                        cmdline = ["./build/poseidon-distributed",
...
try:
    ...
    pushback = InterestDdosAttack (name="attack-pushback-distributed",
                                   algorithms = ["pushback-distributed"],
                                   topologies = ["vergleich-pitlast"],
                                   evils = [3],
                                   good  = 0,
                                   runs = range(1,2),
                                   folder = "attackPushback-Distributed",
                                   producer = "gw",
                                   defaultRtt = "80ms")
    pushback.run ()

    SatisfactionMinManFigures (name="figure6", scenario=pushback).run ()
    SatisfactionNumberOfAttackersFigures (name="figure66", scenario=pushback).run ()
