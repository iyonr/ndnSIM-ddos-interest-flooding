...
#include "ddos-app-mitigate.h"
#include <ns3/point-to-point-module.h>

NS_LOG_COMPONENT_DEFINE ("DdosAppMitigateMitigate");

NS_OBJECT_ENSURE_REGISTERED (DdosAppMitigate);

TypeId
DdosAppMitigate::GetTypeId ()
{
  static TypeId tid = TypeId ("DdosAppMitigate")
    .SetParent<App> ()
    .AddConstructor<DdosAppMitigate> ()

    .AddAttribute ("AvgGap", "AverageGap",
		   StringValue ("1ms"),
                   MakeTimeAccessor (&DdosAppMitigate::m_avgGap),
                   MakeTimeChecker ())
    .AddAttribute ("Prefix","Name of the Interest",
                   StringValue ("/"),
                   MakeNameAccessor (&DdosAppMitigate::m_prefix),
                   MakeNameChecker ())
    .AddAttribute ("LifeTime","Interest lifetime",
                   StringValue ("1s"),
                   MakeTimeAccessor (&DdosAppMitigate::m_lifetime),
                   MakeTimeChecker ())
    .AddAttribute ("Evil", "Evil bit",
                   BooleanValue (false),
                   MakeBooleanAccessor (&DdosAppMitigate::m_evilBit),
                   MakeBooleanChecker ())
    .AddAttribute ("DataBasedLimits", "Calculate frequency based on how many data packets can be returned",
                   BooleanValue (true),
                   MakeBooleanAccessor (&DdosAppMitigate::m_dataBasedLimit),
                   MakeBooleanChecker ())
    ;
  return tid;
}

DdosAppMitigate::DdosAppMitigate ()
...
void
DdosAppMitigate::OnNack (const Ptr<const Interest> &interest)
{
  ...
  //       				 &DdosAppMitigate::SendPacket, this);
}

void
DdosAppMitigate::OnData (const Ptr<const Data> &data)
{
  // who cares
}

void
DdosAppMitigate::SendPacket ()
{
  m_seq++;
  // send packet
...
void
DdosAppMitigate::StartApplication ()
{
...
void
DdosAppMitigate::StopApplication ()
{
  m_nextSendEvent.Cancel ();

  // std::cerr << "# references before delayed stop: " << m_face->GetReferenceCount () << std::endl;
  Simulator::Schedule (Seconds (10.0), &DdosAppMitigate::DelayedStop, this);
}

void
DdosAppMitigate::DelayedStop ()
{
