...
#include "ddos-app-ipc.h"
#include <ns3/point-to-point-module.h>

NS_LOG_COMPONENT_DEFINE ("DdosAppIPC");

NS_OBJECT_ENSURE_REGISTERED (DdosAppIPC);

TypeId
DdosAppIPC::GetTypeId ()
{
  static TypeId tid = TypeId ("DdosAppIPC")
    .SetParent<App> ()
    .AddConstructor<DdosAppIPC> ()

    .AddAttribute ("AvgGap", "AverageGap",
		   StringValue ("1ms"),
                   MakeTimeAccessor (&DdosAppIPC::m_avgGap),
                   MakeTimeChecker ())
    .AddAttribute ("Prefix","Name of the Interest",
                   StringValue ("/"),
                   MakeNameAccessor (&DdosAppIPC::m_prefix),
                   MakeNameChecker ())
    .AddAttribute ("LifeTime","Interest lifetime",
                   StringValue ("1s"),
                   MakeTimeAccessor (&DdosAppIPC::m_lifetime),
                   MakeTimeChecker ())
    .AddAttribute ("Evil", "Evil bit",
                   BooleanValue (false),
                   MakeBooleanAccessor (&DdosAppIPC::m_evilBit),
                   MakeBooleanChecker ())
    .AddAttribute ("DataBasedLimits", "Calculate frequency based on how many data packets can be returned",
                   BooleanValue (true),
                   MakeBooleanAccessor (&DdosAppIPC::m_dataBasedLimit),
                   MakeBooleanChecker ())
    ;
  return tid;
}

DdosAppIPC::DdosAppIPC ()
...
void
DdosAppIPC::OnNack (const Ptr<const Interest> &interest)
{
  ...
  //       				 &DdosAppIPC::SendPacket, this);
}

void
DdosAppIPC::OnData (const Ptr<const Data> &data)
{
  // who cares
}

void
DdosAppIPC::SendPacket ()
{
  m_seq++;
  // send packet
...
void
DdosAppIPC::StopApplication ()
{
  m_nextSendEvent.Cancel ();

  // std::cerr << "# references before delayed stop: " << m_face->GetReferenceCount () << std::endl;
  Simulator::Schedule (Seconds (10.0), &DdosAppIPC::DelayedStop, this);
}

void
DdosAppIPC::DelayedStop ()
{
