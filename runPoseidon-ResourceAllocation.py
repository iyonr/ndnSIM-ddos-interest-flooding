        ...
        for algorithm in self.algorithms:
            for topology in self.topologies:
                for evil in self.evils:
                    for run in self.runs:
                        cmdline = ["./build/poseidon-resource-allocation",
...
try:
    ...
    resource = InterestDdosAttack (name="attack-resource-allocation",
                                   ...
                                   good  = 0,
                                   runs = range(1,2),
                                   folder = "attackResourceAllocation",
                                   producer = "gw",
                                   defaultRtt = "80ms")
    resource.run ()

    SatisfactionMinManFigures (name="figure6", scenario=resource).run ()
    SatisfactionNumberOfAttackersFigures (name="figure66", scenario=resource).run ()
